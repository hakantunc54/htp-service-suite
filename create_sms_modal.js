...
// 1. We replace handleSmsAction entirely
const oldHandleSmsActionRegex = /const handleSmsAction = async \(templateName: string\) => \{[\s\S]*?\}\s*\}\s*;/m;
const newHandleSmsAction = `const [selectedTemplateName, setSelectedTemplateName] = useState("");
...
code = code.replace(oldHandleSmsActionRegex, newHandleSmsAction);

// 2. We change the button onClick
const btnRegex = /onClick=\{\(\) => handleSmsAction\("Erstkontakt"\)\}/m;
const newBtn = `onClick={openSmsModal}`;
code = code.replace(btnRegex, newBtn);

// 3. We rewrite the Modal UI
const modalRegex = /\{showManualSmsModal && \([\s\S]*?\}\)/m;
