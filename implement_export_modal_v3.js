...
const endIndex = code.indexOf(oldExportEndStr);

if (startIndex !== -1 && endIndex !== -1) {
    const realEndIndex = endIndex + oldExportEndStr.length;
  ...
  const runExportCsv = () => {
    ...
    code = code.substring(0, startIndex) + newExportLogic + code.substring(realEndIndex);
} else {
...
code = code.replace('onClick={() => handleExportCsv(vehicle)}', 'onClick={() => handleExportClick(vehicle)}'); // Multiple
...
// Re-apply CRLF for Windows just to be nice
code = code.replace(/\n/g, '\r\n');

fs.writeFileSync('src/app/planning/page.tsx', code, 'utf8');
console.log("Export Modal implemented successfully with robust LF matching!");
